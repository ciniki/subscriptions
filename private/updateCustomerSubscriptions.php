...
// <rsp stat='ok' id='34' />
//
function ciniki_subscriptions_updateCustomerSubscriptions($ciniki, $tnid, $customer_id, $subs, $unsubs) {
    //
    ...
    foreach($subs as $sid) {
        ...
        if( !isset($subscriptions[$sid]) ) {
            $rc = ciniki_core_objectAdd($ciniki, $tnid, 'ciniki.subscriptions.customer', array(
        ...
        elseif( isset($subscriptions[$sid]) && $subscriptions[$sid]['status'] != 10 ) {
            $rc = ciniki_core_objectUpdate($ciniki, $tnid, 'ciniki.subscriptions.customer', $subscriptions[$sid]['id'],
